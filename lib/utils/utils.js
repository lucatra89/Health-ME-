...
define(function(require) {
	...
	var extend = function(protoProps, staticProps) {
		...
		if (protoProps && _.has(protoProps, 'constructor')) {
			...
			eval("child = function " + protoProps.constructorName
	...
	// We expect the templates module as a Require object module with the
	// following structure:
	// - key: the name of the template as it must be referenced by the Backbone
	// view
	// - value: the path of the html file containing the HTML fragment of the
	// template
	...
	// here we say that Utils is a Backbone object so that it is able to send
	// and receive events
	...
	Utils.loadTemplates = function() {
		...
		// we return this so that the caller can simply call the once method in
		// chain with the call to the loadTemplates method
		return this;
	};
	...
	// function that will be called by the router every time a view must be
	...
	Backbone.View.prototype.close = function() {
		...
		this.remove();
		// remove references to the DOM element of the view (both jQuery and JS
		// objects)
		this.$el = null;
		this.el = null;
	};
	...
	// function that will be called by the router every time a view must be
	// removed from the DOM
	Backbone.Router.prototype.changePage = function(page,title) {

		this.structureView.trigger('clearIcon');
		this.structureView.trigger('clearSearcher');
		//change title
		if(title !== undefined)
			this.structureView.pageTitle.innerHTML= title;
		...
		this.structureView.el.appendChild(page.el);
	...
	Backbone.Router.prototype.alreadyExist = function(page){
		return (page.constructorName == this.currentView.constructorName);
	};



	Backbone.Router.prototype.changeSubViews = function(options) {
		if(options.clearIcon !== false)
			this.structureView.trigger('clearIcon');

		if(this.currentView)
			this.currentView.trigger('changeSubViews', options);
	};

	// Pages
	// -------------

	// Pages are special kinds of Backbone's views that take the whole screen of
	// the mobile application
	// the router is in charge of dynamically navigating among pages by means of
	...
	Utils.showWebsite = function(url) {
		if (navigator.connection.type == Connection.NONE) {
			navigator.notification
					.alert(
							'It looks like you have no Internet connection, please can you check it?',
							function() {
							}, "No Internet");
			return;
		}
		window
				.open(url, '_blank',
						'location=yes,closebuttoncaption=close,EnableViewPortScale=yes');
	};

	// if we have a 404 error when loading an image, we put a transparent pixel
	...
	String.prototype.startsWith = function(prefix) {
		...
		return s.indexOf(prefix.toLowerCase()) === 0;
	};

	// equivalent to Java's String.endsWith
	String.prototype.endsWith = function(suffix) {
		var s = this.toLowerCase();
		return s.indexOf(suffix.toLowerCase(), s.length - s.length) !== -1;
	};
